...
namespace AppAdvisory.ww
{
	...
	public class Autorun
	{
		...
		static void RunOnce() 
		{
			...
			int count = EditorPrefs.GetInt(WelcomeWW.PREFSHOWATSTARTUP + "autoshow",0);
			...
			EditorPrefs.SetInt(WelcomeWW.PREFSHOWATSTARTUP + "autoshow", count + 1);

			WelcomeWW.showAtStartup = EditorPrefs.GetInt(WelcomeWW.PREFSHOWATSTARTUP, 1) == 1;
			 
			if (WelcomeWW.showAtStartup)
			{
				DGChecker.CheckItNow();
			
				WelcomeWW.OpenPopupStartup();
			}
		}
	}
}         